...
## v0.1.2
- WebUI：
  - 修复日志表格表头固定遮挡内容的问题，取消 sticky 效果，表头随内容滚动
  - 优化日志解析逻辑，自动去除消息体中冗余的时间戳和日志级别，使显示更清爽
  - 静态资源增加版本号参数，强制刷新浏览器缓存，确保样式与逻辑更新即时生效
- 版本：CurrentVersion 提升为 v0.1.2

## v0.1.1
- 缓存策略：新增直链缓存过滤模式，支持黑名单（命中不缓存）和白名单（命中才缓存）两种模式切换
- WebUI：
  - 媒体服务器配置弹窗布局优化，“开启内部重定向”复选框移至底部左侧，解决换行与对齐问题
  - 修复 CSS 样式，确保表单控件在不同分辨率下的对齐与显示效果
  - 优化文本域默认高度与响应式布局
- 版本：CurrentVersion 提升为 v0.1.1
